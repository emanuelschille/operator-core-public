...
def test_performance_review_request_flows_through_review_ops() -> None:
    ...
    update_payload = {
        ...
        "message": {
            ...
            "chat": {"id": 4601, "type": "private"},
            "from": {"id": 5601, "username": "julia"},
        },
    }
    ...
    assert result.execution_result.result_summary == "Performance-Review-Stub vorbereitet."
    run = run_repo.get(result.execution_result.run_id)
    assert run is not None
    assert run.output_snapshot["lane_name"] == "review_ops"
    assert run.output_snapshot["action_type"] == "performance_review"
...
def test_next_step_request_flows_through_review_ops() -> None:
    request_flow_service, _, run_repo, _ = build_service()
    update_payload = {
        "update_id": 2602,
        "message": {
            "message_id": 3602,
            "text": "next_step reels lernen",
            "chat": {"id": 4602, "type": "private"},
            "from": {"id": 5602, "username": "julia"},
        },
    }
    ...
    assert result.was_executed is True
    assert result.execution_result is not None
    assert result.execution_result.result_summary == "Next-Step-Stub vorbereitet."
    run = run_repo.get(result.execution_result.run_id)
    assert run is not None
    assert run.output_snapshot["action_type"] == "next_step"


def test_non_review_known_command_is_not_sent_to_review_ops() -> None:
    request_flow_service, _, run_repo, _ = build_service()
    update_payload = {
        "update_id": 2603,
        "message": {
            ...
            "chat": {"id": 4603, "type": "private"},
            "from": {"id": 5603, "username": "julia"},
        },
    }
    ...
    run = run_repo.get(result.execution_result.run_id)
    assert run is not None
    assert run.output_snapshot.get("lane_name") != "review_ops"
