...
def test_page_brief_request_flows_through_funnel_ops() -> None:
    request_flow_service, job_repo, run_repo, event_repo = build_service()
    update_payload = {
        "update_id": 2801,
        "message": {
            "message_id": 3801,
            "text": "/page_brief stillkissen landingpage",
            "chat": {"id": 4801, "type": "private"},
            "from": {"id": 5801, "username": "julia"},
        },
    }
    ...
    assert result.execution_result.result_summary == "Page-Brief-Stub vorbereitet."
    run = run_repo.get(result.execution_result.run_id)
    assert run is not None
    assert run.output_snapshot["lane_name"] == "funnel_ops"
    assert run.output_snapshot["action_type"] == "page_brief"
...
def test_offer_path_request_flows_through_funnel_ops() -> None:
    request_flow_service, _, run_repo, _ = build_service()
    update_payload = {
        "update_id": 2802,
        "message": {
            "message_id": 3802,
            "text": "offer_path stillkissen bis bio link",
            "chat": {"id": 4802, "type": "private"},
            "from": {"id": 5802, "username": "julia"},
        },
    }
    ...
    assert result.was_executed is True
    assert result.execution_result is not None
    assert result.execution_result.result_summary == "Offer-Path-Stub vorbereitet."
    run = run_repo.get(result.execution_result.run_id)
    assert run is not None
    assert run.output_snapshot["action_type"] == "offer_path"


def test_non_funnel_known_command_is_not_sent_to_funnel_ops() -> None:
    request_flow_service, _, run_repo, _ = build_service()
    update_payload = {
        "update_id": 2803,
        "message": {
            "message_id": 3803,
            "text": "/performance_review letzte 7 tage",
            "chat": {"id": 4803, "type": "private"},
            "from": {"id": 5803, "username": "julia"},
        },
    }
    ...
    run = run_repo.get(result.execution_result.run_id)
    assert run is not None
    assert run.output_snapshot.get("lane_name") != "funnel_ops"
